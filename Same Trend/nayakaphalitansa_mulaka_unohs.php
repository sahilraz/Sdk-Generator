...
require_once __DIR__ . '/quantum-connect.php';
		...
		if ($toggle == 1 || !$apiFetchOk) {
			...
			if ($toggle == 1) {
				$manualQuery = $conn->query("SELECT sankhye FROM hastacalita_phalitansa WHERE sthiti='1' LIMIT 1");
				if ($manualQuery && mysqli_num_rows($manualQuery) > 0) {
					...
					$conn->query("UPDATE hastacalita_phalitansa SET sthiti='0' WHERE sthiti='1'");
				}
			}
		...
		$localIssue = '';
		$localIssueRes = $conn->query("SELECT atadaaidi FROM gelluonduhogu ORDER BY kramasankhye DESC LIMIT 1");
		...
		if($gadhipathulidhadi >= 1){
			...
			$tathya = mysqli_query($conn,"INSERT INTO `gellaluhogiondu_phalitansa` (`kalaparichaya`,`bele`,`phalitansa`,`banna`,`phalitansadaprakara`,`dinankavannuracisi`) VALUES ('".$samasyesreni['atadaaidi']."','".$yadrcchikasankhye."','".$yadrcchika."','".$banna."','shonu','".$dinanka."')");
		}
		$tarika = date('Y-m-d H:i:s'); // Current timestamp
		$parabartikrama = $samasyesreni['atadaaidi'] + 1;
    $tathya1 = mysqli_query($conn, "INSERT INTO `gelluonduhogu` (`atadaaidi`, `dinankavannuracisi`) VALUES ('$parabartikrama', '$tarika')");
